...
(() => {
  ...
  function mixedTextWrapRestoreHint(el) {
    if (el.dataset?.impeccableTextWrap !== "true" || !el.parentElement) return null;
  ...
  function componentModuleCandidates(_manifest, modulePath, absPath) {
  ...
  function _scopeCssBlock(css, prefix) {
    let out = "";
    let i = 0;
    while (i < css.length) {
  ...
  function probeJsxWrapperForOrphan(filePath, sessionId, opts) {
    ...
    fetch(url)
      ...
      .catch((err) => {
        const detail = err?.message ? err.message : "fetch failed";
  ...
  function completeSourceInjection(wrapper, sessionId, opts) {
    ...
    expectedVariants = parseInt(wrapper.dataset.impeccableVariantCount || arrivedVariants, 10);
    if (arrivedVariants <= 0) {
      if (state === "GENERATING") {
  ...
  function releaseDiscardedStaticWrappers(sessionId, wrappers) {
    ...
    const set = wrappers?.length ? wrappers : discardedWrappers(sessionId);
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      const expected = parseInt(wrapper.dataset.impeccableVariantCount || "0", 10);
        ...
        case "error":
          ...
          if (awaitingAcceptResult?.id && msg.id === awaitingAcceptResult.id) {
            ...
            const acceptFailDetail = String(msg.message || "unknown error")
              ...
              .replace(/[.!?]?$/, ".");
            showToast(
              `Accept failed: ${acceptFailDetail} The variant may not have been saved. If the change is missing, pick the element and generate again.`,
              8000,
            );
            break;
          }
...
void main() {
  ...
  function resumeSession(recoveryRevision = liveInteractionRevision, opts = {}) {
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.impeccableVariantCount || "0", 10);
    const variants = wrapper.querySelectorAll(
      '[data-impeccable-variant]:not([data-impeccable-variant="original"])',
    );
    arrivedVariants = variants.length;
