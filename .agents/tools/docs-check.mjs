...
if (squad.length !== 9) fail(`esperados 9 agents em .agents/agents, encontrados ${squad.length}`)
...
if (squad.length === 9 && !fails.some((f) => f.includes('.claude/agents'))) pass('9 agents com symlinks íntegros')
...
for (const folder of specFolders) {
	...
	if (isTarget) {
		for (const f of ['spec.md', 'design.md', 'copy.md', 'plan.md']) {
			...
			if (body && /<[A-Za-z][^>\n]{6,}>/.test(body)) warn(`spec ${folder}/${f}: ainda contém placeholders do template`)
		}
	}
}
